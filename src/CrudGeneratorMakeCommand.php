...
use KMsalehi\LaravelEzCrud\Contracts\ViewMaker;
...
class CrudGeneratorMakeCommand extends ControllerMakeCommand
{
    ...
    protected function buildClass($name)
    {
        ...
        if ($model and $this->option('views')) {
            if ($this->viewMaker->makeViews($model, $this->getFillables($model))) {
                $this->line("\n  <bg=blue> INFO </> {$model} Views created successfully.");
            }
        }
    ...
    private function createViews($fillables)
    {
    }
    ...
    protected function getOptions()
    {
        return [
            ...
            ['views', null, InputOption::VALUE_NONE, 'Create view files based on given model'],
        ];
    }
